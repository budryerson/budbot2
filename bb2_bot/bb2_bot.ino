  ...
  *             26 OCT 19 - Planning to switch TFMini interface to I2C
...
#include <EEPROM.h>
#include <Wire.h>          //  I2C Communication device addresses
                           //  TFMini-Plus def addr  0x10
                           //  BNO055_I2C_ADDR1      0x28
                           //  BNO055_I2C_ADDR2      0x29
...
// The goal of the two flags is to create two loops so as to pan
// the Lidar sensor twice as fast as the radio transfers data to
// the joystick and to the PC beyond.
//
//  Output Compare Register 3A Interrupt Service Routine (ISR)
//  OCR3A is set to 3124 (50ms) in 'routines.cpp'
//  !! The loop is running at 0.032sec - 22APR19 !!
//  !! The loop is running at 9-14ms - 29OCT19 !!
volatile bool t3aFlag = false;       //  Timer 3A Flag
ISR( TIMER3_COMPA_vect)    //  Timer 3A triggers ISR every 50ms
{
    t3aFlag = true;        //  set Timer 3A Flag
}

//  Output Compare Register 3B Interrupt Service Routine (ISR)
//  OCR3B is set to 1562 (25ms) in 'routines.cpp'
volatile bool t3bFlag = false;       //  Timer 3B Flag
ISR( TIMER3_COMPB_vect)    //  Timer 3B triggers ISR every 25ms
{
    t3bFlag = true;        //  set Timer 3B Flag
}
/* -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- */
...
void setup()
{
    ...
    playNote( 2);         // play E above middle C 0x5A
}
...
void loop()
{
    //  Every 50.0ms, the Timer3A Interrupt Service Routine sets t3aFlag TRUE.
    //  The last statement of this loop resets the T3A flag to FALSE.
    if( t3aFlag)
    {
        ...
        //  printAllData();  // also in 'routines.cpp', print
                             // three long segments of hex characters
                             // adds 20ms and causes the loop to skip

        //  -  -  -  -  -  -   HOUSEKEEPING  -  -  -  -  -  -  -
        t3aFlag = false;        // Timer3A flag gets reset every 50ms
    ...
    if( t3bFlag)
    {
        // Get Lidar range and flux data and save to rDat1 and save to rDat1
        bbLidar.read();
        t3bFlag = false;         // Timer3B flag gets reset every 25ms
    }
